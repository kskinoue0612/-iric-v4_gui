...
jobs:
  build:
    ...
    - name: Build
      ...
      shell: cmd
      run: |
        @REM Copy latest versions of iriclib
        copy /Y iricsdk\iriclib-${{env.IRICLIB_VERSION}}\src\iric.f90    ${{env.SOURCE_DIR}}\src\.
        copy /Y iricsdk\iriclib-${{env.IRICLIB_VERSION}}\lib\iriclib.lib ${{env.SOURCE_DIR}}\libs\.
        cd ${{env.SOURCE_DIR}}
        .\build.bat
    ...
    - name: Copy to package data directory
      shell: pwsh
      run: |
        Copy-Item .\${{env.SOURCE_DIR}}\INSTALL\* .\${{env.ONLINE_UPDATE_SRC_DIR}}\packages\${{env.PACKAGE_DIR}}\data\.
    ...
    # The secrets.ONLINE_UPDATE_TOKEN is a repository secret (REPO/settings/secrets/actions)
    # which is a copy of a personal access token (settings/tokens) from someone who has write
    # access to the online_update repository
    - name: Commit and push branch
      ...
      with:
        ...
        branch: __${{github.event.repository.name}}_${{github.run_number}}        # @todo change to master after testing
